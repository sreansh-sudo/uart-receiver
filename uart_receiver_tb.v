...
module uart_receiver_tb;
    ...
    // -------------------------------------------------------
    // UART frame transmitter task (LSB first, no parity)
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // Monitor — log every received byte
    // -------------------------------------------------------
    always @(posedge clk) begin
        if (data_ready) begin
            rx_count <= rx_count + 1;
            $display("RX[%0d] @ %0t ns = 0x%h", rx_count + 1, $time, data_out);
        end
    end
    ...
    initial begin
        ...
        // Hold reset for 10 cycles, then release
        #(10 * CLK_PERIOD);
        rst_n = 1'b1;
        #(5  * CLK_PERIOD);
        ...
        #(BIT_TIME);            // inter-frame gap
        send_uart_frame(8'h2F);

        // Wait long enough for the second byte to fully complete
        #(3 * BIT_TIME);

        $display("Final data_out = 0x%h", data_out);
